...
static Position scaled(Position center, Position pos, Position spreaded, float scale) {
	return Position(
...
{
	// read points positions to per-axis arrays
	std::vector<float> xPos;
	std::vector<float> yPos;
	std::vector<float> zPos;

	xPos.reserve(input->numSamples);
	yPos.reserve(input->numSamples);
	zPos.reserve(input->numSamples);

	
	for (int i = 0; i < input->numSamples; i++) {
		xPos.push_back(input->getChannelData(0)[i]);
		yPos.push_back(input->getChannelData(1)[i]);
		zPos.push_back(input->getChannelData(2)[i]);
	}

	{
		auto it = unique(xPos.begin(), xPos.end());
		xPos.resize(distance(xPos.begin(), it));
	}
	{
		auto it = unique(yPos.begin(), yPos.end());
		yPos.resize(distance(yPos.begin(), it));
	}
	{
		auto it = unique(zPos.begin(), zPos.end());
		zPos.resize(distance(zPos.begin(), it));
	}
	...
	for (float xPosIt : xPos) {
		for (float yPosIt : yPos) {
			for (float zPosIt : zPos) {
																	 ...
																	 SOP_Output* output) const
{
	...
	std::vector<Position> samples(input->numSamples);
	for (int i = 0; i < input->numSamples; i++) {
		Position sample(
